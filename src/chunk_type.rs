...
impl TryFrom<[u8; 4]> for ChunkType {
    ...
    fn try_from(buf: [u8; 4]) -> Result<Self, Self::Error> {
        let ret = ChunkType { bytes: buf };
        if ret.bytes[0].is_ascii_alphabetic()
            && ret.bytes[1].is_ascii_alphabetic()
            && ret.bytes[2].is_ascii_alphabetic()
            && ret.bytes[3].is_ascii_alphabetic()
        {
            return Ok(ret);
        }
...
impl FromStr for ChunkType {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let b: [u8; 4] = s.as_bytes().try_into()?;
